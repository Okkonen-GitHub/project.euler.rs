fn main() {
    ...
    let horizontal =  check_horizontal(&nums, 4);
    println!("horizontal: {} ", horizontal);
    let vertical = check_vertical(&nums, 4);
    println!("vertical:   {} ", vertical);
    let diagonal = check_diagonal(nums, 4);
    println!("diagonal:   {}", diagonal);
}

// ! n will always be 4

fn check_horizontal(nums: &[[u64; 20]; 20], n: usize) -> u64 { // simply check each row
    let mut largest_product = 0;
    for row in nums {
        for i in 0..row.len() -n +1 {
            let mut largest = 1;
            for value in &row[i..i+n] { // on each row check every adjacent n (4) numbers
                // println!("made it to here {}", value);
                largest *= *value;
            }
...
fn check_vertical(nums: &[[u64; 20]; 20], n: usize) -> u64 { // this is super messy..
    let mut largest_product = 0;
    for i in 0..nums.len() {
        for j in 0..(nums.len() - n + 1) {
            let mut largest = 1;
            for value in &nums[j..j+n] {
                largest *= value[i];
            }
            if largest > largest_product {
                largest_product = largest;
